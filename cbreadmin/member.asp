...
<thead>
          <tr > 
            ...
            <th width="60" class=""><strong>昵称</strong></th>
            <th width="70" class=""><strong>电话</strong></th>
            <th width="70"  class="hidden-phone"><strong>注册时间</strong></td>
            <th width="50" class=""><strong>登录时间</strong></th>
          ...
          <%do while not rs.eof%>
          <tr >
            <td class=""><%=rs("username")%></td>
            <td class=""><%=rs("usermobile")%></td>
            <td class="hidden-phone"><%=rs("regtime")%></td>

            <td class="hidden-phone"><%=rs("logtime")%></td>
...
if action="Del" then
conn.execute "delete from [member] where id="&Trim(delid)
response.Redirect("member.Asp?sta=del")
end if
%>
